...
import type { Coin, CoinInventory, MachineState, Product } from '../types/index'
...
const VendingMachine: React.FC = () => {
  ...
  const [returnedCoins, setReturnedCoins] = useState<CoinInventory | null>(null)
    ...
    const handleSelectProduct = (product: Product) => {
      ...
      const changeMsg = changeAmount > 0 ? `Change: ${changeAmount}¢` : 'No Change'
    ...
    <div>
      ...
      <div>
        ...
        {(['nickel', 'dime', 'quarter'] as Coin[]).map((coin) => (
          <CoinButton key={coin} type={coin} onClick={() => handleDeposit(coin)} />
        ))}
      </div>
      <div>
        <h2>Select your drink!</h2>
        {products.map((product) => (
          <ProductSlot key={product.name} product={product} onClick={() => handleSelectProduct(product)} />
        ))}
      </div>
      <ControlPanel
